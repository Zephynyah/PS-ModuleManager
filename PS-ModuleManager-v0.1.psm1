...
function Get-PSMMRemoteModules {
    ...
    .DESCRIPTION
        ...
        on each target.  Returns ModuleInfo objects with version comparison status.
    .PARAMETER ComputerNames
    ...
    Write-PSMMLog -Severity 'INFO' -Message "Inventorying modules on $($ComputerNames.Count) computer(s) ..."

    $inventoryScript = {
        param($Computer, $Cred)
        try {
            $splat = @{ ComputerName = $Computer; ScriptBlock = { Get-Module -ListAvailable | Select-Object Name, @{N = 'Version'; E = { $_.Version.ToString() } }, ModuleBase } }
    ...
    $jobs = Invoke-PSMMParallel -ComputerNames $ComputerNames -ScriptBlock $inventoryScript -ArgumentList @($Credential)
    return $jobs
}
...
function Register-PSMMMainWindowEvents {
    ...
    $btnInventory.Add_Click({
            ...
            Write-PSMMLog -Severity 'INFO' -Message "Starting inventory on $($selected.Count) computer(s) ..."

            # Launch async inventory
            $null = Get-PSMMRemoteModules -ComputerNames $selected
