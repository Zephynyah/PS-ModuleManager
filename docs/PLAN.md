...
```
PS-ModuleManager.psm1            <-- everything lives here (~3585 lines)
|-- #region Assembly Loading       (PresentationFramework, WindowsBase, ComputerItem/ModuleGridItem C# classes)
|-- #region Script-Scoped State    ($script:SyncHash, $script:Settings, ObservableCollections, timer refs)
...
|-- #region Credential Management  (Show-PSMMCredentialDialog, Get-PSMMCredential)
|-- #region WPF XAML Definition    (inline XAML here-strings: credential dialog, main window, settings dialog)
|-- #region SyncHash Helpers       (Initialize-PSMMSyncHash, Invoke-PSMMDispatcherUpdate, Invoke-PSMMBackgroundRunspace)
|-- #region WPF Helpers            (New-PSMMWindow, Find-PSMMControl, Invoke-PSMMSafeAction)
|-- #region WPF Event Handlers     (Register-PSMMMainWindowEvents -- all button/menu/keyboard wiring)
|-- #region Job Poller             (Start-PSMMJobPoller -- DispatcherTimer, in-place grid updates)
|-- #region ADSI Helper            (Get-ADSIInfo -- domain/OU discovery helper, exported)
...
| `AppSettings` | `DomainLdapPath`, `OuFilter`, `ModuleSearchPaths`, `CentralSharePath`, `MaxConcurrency`, `CredentialMode`, `LogPath`, `RetryCount`, `ReachabilityCheck`, `LogLevel`, `JobTimeoutSeconds`, `ExcludeServers`, `ExcludeVirtual`, `GlobalExcludeList`, `OSFilter` |
...
| `PS-ModuleManager.psm1` | **Single comprehensive module** -- all code, inline WPF XAML, and logic (~3585 lines). |
...
- **PSGallery integration** -- Support `Install-Module` / `Find-Module` from PowerShell Gallery as an alternative module source, with a toggle in settings.
- **HTML report generation** -- Styled HTML inventory diff report for email distribution or intranet publishing.
- **Scheduled task wrapper** -- `Register-PSMMScheduledTask` for unattended inventory scans and email reports.
- **Module version pinning** -- Pin specific versions per computer group for staged rollouts.
- **Dark/Light theme toggle** -- Extract color resources into a `ResourceDictionary` for easy theme swapping.
- **Authenticode enforcement** -- Add `RequireSignedModules` setting to validate `Get-AuthenticodeSignature` before extraction.
- **Connection status caching** -- Cache WinRM reachability results with configurable TTL to avoid re-testing on every operation.
- **PowerShell 7 / cross-platform** -- Replace WPF with Avalonia or terminal UI for non-Windows support.
